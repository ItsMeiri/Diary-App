...
const UserEntries = (props) => {
	...
	return foundUser ? (
		<React.Fragment>
			<Container className="profile_item">
				<Item.Group>
					<Item>
						<Item.Image
							size="small"
							wrapped
							src={foundUser.picture_uri}
						/>
						<Item.Content>
							<Item.Header className="profile_name">
								{foundUser.name}'s Favorites
							</Item.Header>
							<Item.Description className="profile_description">
								Birthday:{' '}
								{new Date(foundUser.birthday)
				...
				<div className="user-control-wrapper">
					...
					<Button
						className="user-control-button"
						as={Link}
						to={{ pathname: `/user/${foundUser._id}` }}
						inverted
					>
						My Profile
					</Button>
			...
			<Container text textAlign="right">
				<div>
					...
					<Dropdown
						inline
						options={filterArrOptions}
						defaultValue={filterArrOptions[3].value}
						onChange={(e, { value }) => setFilterArr(value)}
					/>
				</div>
			</Container>
			<Container>
				{!foundUser.favorites.filter((entry) =>
					filterArr !== 'all'
						? filterArr === entry.feeling
						: true
				).length ? (
					<Message
						size="huge"
						content="There are no favorites"
					/>
				) : (
					foundUser.favorites
						.filter((entry) =>
		...
		<PageHeader
			...
			desc="The requested user page is not available"
			icon="user close"
		/>
	);
};
