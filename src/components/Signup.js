...
const Signup = props => {
	...
	const schema = {
		...
		picture_uri: Joi.string().min(6).max(1024).uri().allow("").label("Picture"),
		birthday: Joi.date().max("now").required().label("Birthday"),
	};
	...
	function doSubmit(e) {
		...
		if (formErrors) {
			...
			setSubmit(true);
			if (values["picture_uri"] === "") delete values["picture_uri"];
	...
	return (
		<React.Fragment>
			<Header as="h1" icon textAlign="center" size="huge" style={{ marginTop: "1em", color: "#6391D6" }}>
				...
				<Header.Subheader>
					<strong>Sign up to start managing your jornal now!</strong>
				</Header.Subheader>
			</Header>
			<Container as="div" text>
				<Form method="POST" loading={submit} noValidate autoComplete="off" onSubmit={doSubmit}>
					...
					{submitSuccess && <Message positive icon="user plus" content="User created!" />}
					...
					<Divider hidden />
					<Input onChange={handleInputChange} required fluid size="large" label="Name *" type="text" name="name" placeholder="Full Name" defaultValue={values["name"]} />
					{errors["name"] && <span style={errorStyle}>{errors["name"]}</span>}
					{/* <Input required fluid size="large" label="Name *" type="text" name="name" ref={register} /> */}
					<Divider hidden />
					<Input onChange={handleInputChange} required fluid size="large" label="Birthday *" type="date" name="birthday" defaultValue={values["birthday"]} />
					...
					<Divider hidden />
					<Input onChange={handleInputChange} fluid size="large" label="Picture URL" type="text" name="picture_uri" placeholder="Profile Picture (URL)" defaultValue={values["picture_uri"]} />
					{errors["picture_uri"] && <span style={errorStyle}>{errors["picture_uri"]}</span>}
